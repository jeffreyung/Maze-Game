...
public class MapGeneratorHard implements MapInterface {
	...
	/**
	 * Constructs the hard map generator.
	 */
	...
	private void setSection6() {
		...
		Section section = new Section(new Position(poX, 7), RIGHT);
		sections.add(section);
			map.get(7).set(poX, ' ');
			map.get(7).set(poX + 1, ' ');
			map.get(7).set(poX + 2, ' ');
	...
	private void setPath() {
		...
		for(int sectCounter = 0; sectCounter < sectionNum; sectCounter += 2) {
			if(sections.get(sectCounter) == null) break;
			Section left = sections.get(sectCounter);
			for(int rightSectCounter = 1; rightSectCounter < sectionNum; rightSectCounter += 2) {
	...
	public void setBox() {
		for(Position p : vertiPath) {
			...
			for(int count = 0; map.get(p.getY() + count).get(p.getX()) != '|'; count ++) {
				...
				if(NoSection5) {
					if(sections.get(2).getOrientation() == RIGHT) {
						if(sections.get(0).getOrientation() == DOWN) {
							if(poY == 6) continue;
						}
					} else if(sections.get(2).getOrientation() == DOWN) {
						if(sections.get(0).getOrientation() == RIGHT) {
							if(poY == 6) continue;
						} else if(sections.get(0).getOrientation() == DOWN) {
							if(poY == 7) continue;
						}
					}
				} else {
		...
		for(int i = 0; i < boardSize; i ++) {
			for(int j = 0; j < boardSize; j ++) {
				if(map.get(i).get(j).equals('B')) map.get(i).set(j, ' ');
			}
		}
	}
	...
	public void setGoals() {
		...
		if(sections.get(lastSection).getOrientation() == RIGHT) {
			...
			map.get(poY + 1).set(poX + 2, 'x');
		} else if(sections.get(lastSection).getOrientation() == DOWN) {
			map.get(poY + 2).set(poX, 'x');
			map.get(poY + 1).set(poX + 1, 'x');
			map.get(poY + 2).set(poX + 1, 'x');
		}
	...
	public void removeWalls() {
		int poX = sections.get(lastSection).getPosition().getX();
		int poY = sections.get(lastSection).getPosition().getY();
		//if(map.get(poY - 1).get(poX - 1).equals('|')) {
		//	map.get(poY - 1).set(poX - 1, ' ');
		//}
		if(map.get(poY - 1).get(poX + 1).equals('|')) {
			map.get(poY - 1).set(poX + 1, ' ');
		}
		if(map.get(poY - 1).get(poX).equals('|')) {
			map.get(poY - 1).set(poX, ' ');
		}
	}
